...
//  Todos los gráficos usan este rango — nada de histórico
// ══════════════════════════════════════════════════════════════
$lunesActual   = date('Y-m-d', strtotime('monday this week'));
$domingoActual = date('Y-m-d', strtotime('sunday this week'));

// ── Totales por categoría — GASTOS esta semana (para Pastel) ──
// El filtro de fecha va en el JOIN para no excluir categorías sin
// transacciones esta semana (así HAVING total > 0 las filtra limpio)
...
// ── Totales por categoría — INGRESOS esta semana (para Anillo) ─
...
// ── Generar los 7 días fijos Lun … Dom ───────────────────────
$diasSemana = [];
for ($i = 0; $i < 7; $i++) {
...
/**
 * Mezcla resultados de BD con los 7 días fijos.
 * Días sin transacciones quedan en 0.
 */
...
// ── GASTOS por día esta semana (para gráfico Lineal) ─────────
...
// ── INGRESOS por día esta semana (para gráfico de Barras) ────
...
// ── Totales de la semana (para las tarjetas superiores) ───────
...
jsonOk([
    'gastos'           => $datosGastos,     // categorías esta semana → Pastel
    'ingresos'         => $datosIngresos,   // categorías esta semana → Anillo
    'gastos_por_dia'   => $gastosPorDia,    // 7 días → Lineal
    'ingresos_por_dia' => $ingresosPorDia,  // 7 días → Barras
