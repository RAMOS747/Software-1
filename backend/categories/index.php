...
if ($method === 'POST') {
    ...
    // Verificar máximo 8 categorías por tipo
    $count = $pdo->prepare(
        'SELECT COUNT(*) FROM categorias WHERE usuario_id = ? AND tipo = ?'
    );
    ...
    // Verificar duplicado
    $dup = $pdo->prepare(
        'SELECT id FROM categorias WHERE usuario_id = ? AND nombre = ? AND tipo = ?'
    );
    ...
    $ins = $pdo->prepare(
        'INSERT INTO categorias (usuario_id, nombre, color, tipo) VALUES (?, ?, ?, ?)'
    );
...
if ($method === 'PUT') {
    ...
    $dup = $pdo->prepare(
        'SELECT id FROM categorias WHERE usuario_id = ? AND nombre = ? AND tipo = ? AND id != ?'
    );
...
if ($method === 'DELETE') {
    ...
    // Verificar pertenencia
    $own = $pdo->prepare('SELECT id, tipo FROM categorias WHERE id = ? AND usuario_id = ?');
    $own->execute([$id, $user['id']]);
    if (!$own->fetch()) jsonError('Categoría no encontrada', 404);

    // Eliminar (las transacciones se borran en cascada)
